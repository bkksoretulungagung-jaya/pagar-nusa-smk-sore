...
const SEEN_KEY='pnAdminPasswordBackendV3Seen';
...
try{localStorage.removeItem('pnAdminPasswordBackendMode')}catch(_){}
...
function jsonp(action,payload={},timeout=12000){
  return new Promise((resolve,reject)=>{
    const cb='pnAdminPassV3_'+Date.now()+'_'+Math.random().toString(36).slice(2).replace(/\W/g,'');
...
async function postReliable(action,payload={},timeout=45000){
  const rid='adminpass3-'+Date.now()+'-'+Math.random().toString(36).slice(2);
...
async function probeBackend(){
  try{
    ...
    backendReady=!!(r&&r.ok&&String(r.adminPasswordVersion||'')==='3');
    passwordConfigured=!!(backendReady&&r.adminPasswordConfigured===true);
    capabilityResolved=true;
    if(backendReady){
      localStorage.setItem(SEEN_KEY,'1');
      if(passwordConfigured)localStorage.setItem(CONFIG_KEY,'1');
      else localStorage.removeItem(CONFIG_KEY);
    }
    updateUiState();
    return {ready:backendReady,configured:passwordConfigured};
  }catch(_){
    capabilityResolved=true;
    backendReady=false;
    passwordConfigured=localStorage.getItem(CONFIG_KEY)==='1';
    updateUiState();
    return {ready:false,configured:passwordConfigured};
  }
}
...
window.submitAdminLogin=async function(ev){
  ...
  let cap={ready:false,configured:localStorage.getItem(CONFIG_KEY)==='1'};
  try{cap=await Promise.race([getCapability(),sleep(5000).then(()=>cap)])}catch(_){}

  if(cap.ready&&cap.configured){
    try{
...
function updateUiState(){
  ...
  if(!st)return;
  if(backendReady&&passwordConfigured){st.className='pnAdminPassSafeState ready';st.textContent='✓ Password server sudah aktif. Login menggunakan password admin yang tersimpan di server.';if(btn)btn.disabled=false}
  else if(backendReady){st.className='pnAdminPassSafeState ready';st.textContent='✓ Server siap. Saat ini login masih menggunakan password lama sampai password baru berhasil disimpan.';if(btn)btn.disabled=false}
  else if(capabilityResolved&&passwordConfigured){st.className='pnAdminPassSafeState wait';st.textContent='Server password admin sedang tidak dapat dihubungi. Untuk keamanan, login server tidak diturunkan ke password lama.';if(btn)btn.disabled=true}
  else if(capabilityResolved){st.className='pnAdminPassSafeState wait';st.textContent='Backend v3 belum aktif. Login lama tetap aman dan tetap dapat digunakan.';if(btn)btn.disabled=true}
  ...
  if(m&&!m.dataset.bound){
    ...
    $('pnAdminPassSafeForm').addEventListener('submit',async e=>{
      ...
      try{
        ...
        setStatus('ok','✓ Password berhasil diubah. Login berikutnya akan menggunakan password baru.');
