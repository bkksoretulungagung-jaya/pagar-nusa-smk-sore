...
function openPublicInfo(item){
  ...
  modal.innerHTML=`<div class="pnCmsPublicModalCard"><div class="pnCmsPublicModalHead"><button class="pnCmsPublicModalClose" type="button">×</button><h3>${esc(item.title)}</h3><p>${esc(item.date||'')} · ${esc(item.badge||item.type||'INFORMASI')}</p></div><div class="pnCmsPublicModalBody">${esc(item.body||item.summary||'')}</div></div>`;
